...
class ThreeManager(models.Manager):
    def validate_appointment(self,post_data):
        ...
        if len(post_data['travel_from']) < 1:
            errors.append('Must have a starting location')
        if len(post_data['travel_to']) < 1:
            errors.append('Must have an ending location')
        if post_data['travel_from'] > post_data['travel_to']:
            errors.append('Start date must be after end date')
